...
export function ChatScreen({navigation, route}){
  ...
  const getFormatMessage = (text: string = "", image: string = ""): IMessage => {
    return {
    ...
    try{
      ...
      if (bot.model === "text"){
        ...
        for (let imageBase64 of response){
          const image = await imageBase64ToImage(imageBase64, Crypto.randomUUID())
  ...
  const onSend = useCallback(async (messages: IMessage[]) => {
    ...
    const {_id, text, createdAt, image, user_id, name} = getMessageFomat(messages[0])
    await saveMessage(db, tablename, {_id, text, createdAt, image, user_id, name})
  },[])

  const onSendMessageAndRequestResponse = useCallback(async (messages: IMessage[]) =>{
