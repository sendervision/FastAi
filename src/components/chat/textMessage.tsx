...
export function TextMessage({children, ...props}) {
  ...
  const renderTextTripleBactick = (text: string) => {
    const regex = /```([^`]*)```/
  ...
  const TEXT = children?.split('\\n').map((text, index) => {
    return <ParsedText
      ...
      parse={
        [
          {type: 'url', style: {color: theme.colors.primaryContainer, ...styles.url}, onPress: toggleOpenBrowser},
          {type: 'phone', style: {color: theme.colors.primaryContainer, ...styles.url}, onPress: togglePhonenumber},
          {type: 'email', style: {color: theme.colors.primaryContainer, ...styles.url}, onPress: toggleEmail},
          {
            pattern: /\*\*(.*?)\*\*/g, 
            style: {}, 
            onPress: () => {}, 
          ...
          },
          {
            ...
            onPress: () => {}, 
            renderText: (text, matches) => {renderTextTripleBactick(text)}
          },
